...
const ContentDetailView: NextPage = () => {
  ...
  const [fetchFacilityResult, setFetchFacilityResult] = useState<Facility[]>(
    []
  );
  ...
  useEffect(() => {
    const config: AxiosRequestConfig = {
      method: "get",
      url: encodeURI(
        "https://60c6eb8f19aa1e001769feaf.mockapi.io/facilities?id=" + id
      ),
      timeout: 10000,
    };
    ...
    const bookingConfig: AxiosRequestConfig = {
      method: "get",
      url: encodeURI(
        "https://60c6eb8f19aa1e001769feaf.mockapi.io/bookings?facilityId=" + id
      ),
      timeout: 10000,
    };
        ...
        {isLoading ? (
          ...
          fetchFacilityResult.length > 0 ? (
            fetchFacilityResult.map(
              ({ id, name, description, location, imageUrl, rating, rate }) => (
                <Box paddingX={3}>
                  <Center paddingTop={3}>
                  ...
                  <Box width={"100%"}>
                    <Image
                      borderRadius="lg"
                      objectFit="fill"
                      src={error ? "/notAvail2.png" : imageUrl}
                      alt={name}
                      fallback={<Spinner />}
